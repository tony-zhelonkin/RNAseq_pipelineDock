...
REF_FLAT_FILE="/mouse_genome/ref_genome/GCF_000001635.27_GRCm39_genomic.reflat"  # Path to refFlat file for Picard
BED_ANNOTATION_FILE="/mouse_genome/ref_genome/GRCm39_RfSeq.bed"  # BED file for RSeQC infer_experiment.py
...
run_picard() {
    ...
    picard CollectRnaSeqMetrics \
        ...
        RIBOSOMAL_INTERVALS="/mouse_genome/ref_genome/ribosomal.interval_list" \
        > "$log_file" 2>&1
    if [[ $? -ne 0 ]]; then
...
run_picard_mark_duplicates() {
    ...
    echo "Running Picard MarkDuplicates on $bam_file..."
